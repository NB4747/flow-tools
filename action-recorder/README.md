...
- **单条复制** — 每条记录旁边有一个复制按钮，一键复制该选择器
- **全量导出** — "Copy All (JSON)" 按钮将所有记录导出为格式化的 JSON 数组
- **清空** — "Clear" 按钮一键清空所有记录
...
| **复制全部** | 点击 `⎘ Copy All (JSON)` 将所有记录以 JSON 格式复制到剪贴板 |
...
- **Playwright / Puppeteer 脚本**：`page.click('selector')`
- **Selenium 脚本**：`driver.find_element(By.CSS_SELECTOR, 'selector')`
- **数据分析**：导入 Excel / Pandas 做点击热力分析
- **Bug 报告**：粘贴选择器给开发人员，精确描述"点哪里出了问题"
...
这是因为 `index.html` 需要动态生成 bookmarklet 的 `href` 属性：它读取内嵌的源码 → 去除注释/压缩空白 → `encodeURIComponent` 编码 → 拼接为 `javascript:...` → 设置到按钮上。
